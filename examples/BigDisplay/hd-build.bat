...
if [%~1]==[-w]        ( set _hd-warnings=%~2 && shift /1 && shift /1 && goto :restart )
if [%~1]==[--warn]    ( set _hd-warnings=%~2 && shift /1 && shift /1 && goto :restart )
...
if DEFINED _help (
  ...
  echo.  -w, --warn <level>  set warning level to none, default, more or all
  echo.
...
if DEFINED _hd-compile (
  ...
  @REM arduino-cli compile %_hd-sketch% ^
  @REM --fqbn %_hd-fqbn% --build-property %_hd-properties% ^
  @REM --build-cache-path %_hd-cachedir% ^
  @REM --build-path ./build ^
  @REM --warnings %_hd-warnings% ^
  @REM --log --no-color --log-level %_hd-compile-level% >> %_hd-logfile% 2>&1
  @REM --dump-profile 
  echo.
)
